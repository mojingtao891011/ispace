...
#warning IP
           // HTTPLogInfo(@"%@: Started HTTP server on IP addr %@", THIS_FILE, [asyncSocket localHost]);
			...
			NSString *addressIP = [NSString stringWithFormat:@"http://%@:%d" , [asyncSocket localHost] , [asyncSocket localPort]];
            [[NSNotificationCenter defaultCenter]postNotificationName:ADDRESS_IP object:addressIP];
			isRunning = YES;
			[self publishBonjour];
		}
		else
